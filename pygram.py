...
try:
    ...
    or second == password or 
    trird == password):
        while True:
            print(Fore.MAGENTA)
            print(images.home)
            print("+ IDE(IDE) + Command Prompt python ready(CPP) + help coing + training")
            print("%d" % currentDT.hour,":%d" % currentDT.minute,":%d" % currentDT.second)
            main = input()
            ...
            if main.lower() == "help":
                ...
                print("can't make if statment, define, input, import")
                x = input("what seems to be the porblem : ")
                if x == "if statment":
                    print("ok so if statments are not if __init__ is: or not its 'if __init == 'hi':       print('hello world')")
                if x == "define":
                    print("so defining are like def __init__():\n    print('hello')\n'__init__()' = esseansal!")
                if x == 'input':
                    print("so x = input('type something here : ')\n(float) x = float(input('type a float here : '))\nintiger x = int(input('type an int here : '))")
                if x == "import":
                    print("Ok so if you find anything on the internet do a pip so pip install name if its yours i need to be on the same folder")
